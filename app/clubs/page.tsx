...
import { clubShieldUrl } from "@/app/lib/club-shields";
    ...
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
      ...
      {state && <>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(300px,.75fr)]">
          <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex items-baseline justify-between gap-3"><h2 className="text-xl font-black">Listado de clubs</h2><span className="text-sm text-neutral-500">{clubs.length} resultados</span></div><ul className="mt-4 divide-y divide-neutral-200">{clubs.map((club) => <li key={club.id}><button onClick={() => setSelected(club)} className={`flex w-full items-center gap-4 px-2 py-4 text-left ${selected?.id === club.id ? "rounded-lg bg-red-50" : "hover:bg-neutral-50"}`}><span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-neutral-100 text-xs font-black text-neutral-600">{clubShieldUrl(club.id) ? <img src={clubShieldUrl(club.id)!} alt="" className="h-full w-full object-contain p-1"/> : club.id.slice(-2)}</span><span><b className="block">{club.name}</b><small className="text-neutral-600">{club.id} · {club.city}, {club.country}{!club.active && " · Inactivo"}</small></span></button></li>)}</ul>{!clubs.length && <p className="py-10 text-center text-neutral-500">Todavía no hay clubs publicados para esta búsqueda.</p>}</section>
          ...
          {selected ? <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-neutral-100 text-sm font-black text-neutral-600">{clubShieldUrl(selected.id) ? <img src={clubShieldUrl(selected.id)!} alt={`Escudo de ${selected.name}`} className="h-full w-full object-contain p-2"/> : selected.id.slice(-2)}</span><div><p className="text-xs font-bold tracking-[.16em] text-red-700">FICHA DE CLUB</p><h2 className="mt-1 text-2xl font-black">{selected.name}</h2><p className="mt-1 text-sm text-neutral-600">{selected.id}</p></div></div><button aria-label="Cerrar ficha" onClick={() => setSelected(null)} className="rounded-lg bg-neutral-100 px-3 py-1">×</button></div><dl className="mt-5 grid gap-3 text-sm"><div><dt className="font-semibold text-neutral-500">Ubicación</dt><dd>{[selected.city, selected.province, selected.region, selected.country].filter((value) => value && value !== "—").join(" · ")}</dd></div>{selected.founded_text && <div><dt className="font-semibold text-neutral-500">Fundación</dt><dd>{selected.founded_text}</dd></div>}</dl><div className="mt-5 flex flex-wrap gap-3"><a className="rounded-lg bg-neutral-950 px-3 py-2 text-sm font-bold text-white" href={selected.maps_url} target="_blank" rel="noreferrer">Ver ubicación</a>{selected.website && <a className="rounded-lg bg-red-700 px-3 py-2 text-sm font-bold text-white" href={selected.website} target="_blank" rel="noreferrer">Web del club</a>}</div></section> : <section className="rounded-2xl border border-dashed border-neutral-300 p-6 text-sm text-neutral-600">Selecciona un club para ver su ficha pública.</section>}</aside>
        </div>
      </>}
    </div>
  </main>;
